...
ต่อจากบล็อกที่แล้ว เรามาต่อกันที่เรื่องของ Scope นะครับ จริงๆเรื่อง Scope เป็นเรื่องพื้นฐานบน Javascripts มากๆเพราะเป็นเรื่องของการทำงานของโปรแกรมกันเลยทีเดียว แต่ก่อนหน้า เราต่องมาทำความเข้าใจเรื่องการวิธีการทำงานของตัว Javascript Engine กันก่อนนะครับ ว่าตอนที่เรารันโปรแกรมนั้น มันทำอะไรบ้าง

## Javascripts Engine & Friends

เมื่อเราทำการรันตัวไฟล์ Javascripts บน Runtime อะไรก็ตามแต่ จะมี 3 ส่วนหลักๆที่ทำหน้าที่รันโค้ดด้วยกัน คือ
...
Right Hand Side คือค่าที่อยู่ฝั่งขวามือของ `=` ซึ่งจะเป็นการหา Value และรัน Operator ต่างๆ
...
LHS คือ `a` และ `b` คือการตัวแปรที่เราจะเข้าไปเปลี่ยนแปลงค่า<br>
RHS คือ `10` และ `a + 2` ซึ่งจะไปทำการดึงค่าของตัวแปรและรันคำสั่ง เพื่อทำการจัดเก็บไปที่ LHS
...
Engine:		มีการเรียกใช้งานตัวแปร a ต้องไปถาม scope ละ ขอที่อยู่ตัวแปร a หน่อย
Scope: 		หาตำแหน่งของ a และส่งกลับไปให้ engine
Engine: 	ได้ตำแหน่งมาแล้วก็ไปดึงค่า a แล้วทำงานต่อ b = 10 + 2;
```

หลักการคร่าวๆก็จะประมานนี้เพื่อให้ได้เห็นภาพกันนะครับ คงพอจะได้เห็นได้ว่า Compiler และ Engine นั้นมีหน้าที่คอมไพล์และรันโปรแกรม แต่ `Scope` นั้นทำหน้าที่หาตำแหน่งที่อยู่ของตัวแปรในโปรแกรมให้กับ Engine แต่อาจจะสงสัยว่าทำไมต้องหาด้วยละ ก็น่าจะมีที่เดียวอยู่แล้ว เดี๋ยวเรามาดูรายละเอียดกันครับ

## Lexical Scope

Scope เป็นพื้นที่ในการใช้ตัวแปร ซึ่งใน Javascripts เราจะแบ่ง Scope ด้วยเครื่องหมายปีกกา `{}` โดยพื้นที่นอกสุด เป็น Global Scope
...
แต่เราจะเห็นได้ว่า var ที่ประกาศนั้นเป็นการประกาศนอก Block Scope ดังนั้นหากเราใช้ asynchronous ฟังก์ชันใน loop เช่น `setTimeout` และเรียกใช้ตัวแปรนั้น ค่าของตัวแปรจะไม่ถูกต้อง
...
เนื่องมาจากฟังก์ชันนั้นไม่ได้ในรันในทันที เมื่อฟังก์ชันนั้นถูกรันภายหลัง ก็จะมองหาตัวแปร `i` ซึ่งตัวแปร i ใน scope นั้น ซึ่งเป็นค่าที่ถูกบวกไปเรื่อยๆตาม loop ไปแล้ว จึงได้ค่าเป็น 10 นั่นเอง
...
การประกาศตัวแปรที่เกิดขึ้นใน Function Scope นั้นจะอยู่เฉพาะในฟังก์ชันนั้นเท่านั้น รวมไปถึง parameters ของฟังก์ชันด้วย
...
การหาค่าของตัวแปรนั้น จะเป็นการหาค่าแบบ `Down>>Top` คือ หาใน Scope ตัวเองเสร็จ หาไม่เจอ ก็จะค่อยๆหาขึ้นไปใน Parent Scope ขึ้นไปเรื่อยๆจนถึง Global Scope เมื่อเจอตัวแปรที่ Scope ที่ใกล้ที่สุดก็ใช้ตัวแปรนั้นในการรันครั้งนันๆ
...
ใน scope ของ `foo` นั้นมีตัวแปร a อยู่ ดังนั้นเมื่อมีการเรียกตัวแปร a ในฟังก์ชันนี้ Scope ก็จะส่งค่า a ใน scope นี้ไปให้ จึงได้ค่าเป็น a ในขณะที่ scope `bar` นั้นไม่มีตัวแปร a ตัว Scope เองจึงต้องขึ้นไปดูที่ Global Scope เมื่อเจอค่า a ก็ส่งกลับไปให้ ค่า a ใน bar จึงเป็น 1

ทีนี้พอเข้าใจการทำงานของ Scope แล้ว มาลองแก้ปัญหา `setTimeout` ใน loop ตามหัวข้อด้านบนกันดูนะครับ หากจะแก้ปัญหา เราจำเป็นต้องนำ function มาใช้เพื่อให้ได้ Scope ใหม่ภายใต้ loop นั้น เช่น การใช้ IIFE Function
...
จะเห็นได้ว่า เราประกาศตัวแปรใหม่คือ j และ assign ค่าจาก i ให้ จะถูกรันในระหว่าง loop ค่า j จึงเป็นการนับตามค่า i และเนื่องจากเป็นการประกาศ IIFE ฟังก์ชันใหม่ค่า j ของเราในแต่ละรูปนั้น จะเป็นคนละ scope กัน ทำให้ผลรันจึงออกมาถูกต้อง เพราะ Variable Lookup นั้นขึ้นไปหาค่าของตัวแปร `j` ของ scope นั้นได้ถูกต้องนั่นเอง
...
และแน่นอน scope ของ `const` ก็เหมือนกับ let เลย
